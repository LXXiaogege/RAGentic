...
class TextEmbedding:
    def __init__(self, config: AppConfig):
        ...
        if self.use_cache:
            self.cache: EmbeddingsCache = EmbeddingsCache(
                redis_url=self.redis_config.redis_url, ttl=None
            )
            self.logger.info("RedisVL 嵌入缓存已启用并初始化完成。")

    async def _aget_embeddings_from_cache(
        self, text: List[str]
    ...
    async def _acall_api_and_store(
        ...
        response = await self.aclient.embeddings.create(
            model=self.embedding_config.model, input=texts_to_embed
        )
    ...
    async def aget_embedding(self, text: Union[str, List[str]]) -> List[List[float]]:
        ...
        # 1. 批量查找缓存
        cache_entries = (
            await self._aget_embeddings_from_cache(text)
            if self.use_cache
            else [None] * len(text)
        )
        # 2. 区分缓存命中和未命中
        ...
        if texts_to_embed:
            await self._acall_api_and_store(
                texts_to_embed, indices_to_embed, final_results
            )
        else:
