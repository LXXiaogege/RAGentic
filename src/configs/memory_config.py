...
class Mem0Config(BaseModel):
    ...
    vector_store: VectorStoreConfig = Field(
        default_factory=VectorStoreConfig, description="向量数据库配置"
    )
    ...
    history_db_path: str = Field(
        default="data/memory/history.db",
        description="本地 SQLite 数据库备份 Mem0 记忆路径，可从环境变量 MEM0__HISTORY_DB_PATH 加载",
    )
