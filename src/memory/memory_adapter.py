...
def app_embed_to_mem0_dict(
    embed_config, vector_dimension: int = 1024
) -> Dict[str, Any]:
    return {
...
def adapt_appconfig_to_mem0(app_config: AppConfig) -> Dict[str, Any]:
    ...
    embedder_cfg = EmbedderConfig(
        provider="openai",
        config=app_embed_to_mem0_dict(
            app_config.embedding, vector_dimension=app_config.milvus.vector_dimension
        ),
    )

    reranker_config_dict = None
    if mem_specific.enable_rerank:
        reranker_config_dict = {
            "provider": "sentence_transformer",
            "config": app_rerank_to_mem0_dict(app_config.reranker),
        }

    neo4j_cfg = Neo4jConfig(
        url=app_config.neo4j.url,
        username=app_config.neo4j.username,
        password=app_config.neo4j.password,
        database=app_config.neo4j.database,
    ...
    if reranker_config_dict:
        memory_config_dict["reranker"] = reranker_config_dict
