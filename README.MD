...
```python
# 节点处理中
{"node": "retrieve_knowledge", "status": "processing", "state": {...}}
...
| `rewrite_mode` | str | "hyde" | 改写模式：hyde/step-back/sub-query |
| `top_k` | int | 5 | 检索返回数量 |
| `memory_window_size` | int | 10 | 对话记忆窗口大小 |
| `kb_path` | str | - | 知识库目录路径 |
...
| `db_name` | str | - | 远程数据库名称 |
| `collection_name` | str | "knowledge_collection" | 集合名称 |
