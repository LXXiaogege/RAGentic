...
- `api_server.py`：FastAPI 后端，将 `LangGraphQAPipeline` 暴露为 REST/SSE 接口（端口 8000，CORS 允许 `localhost:3000`）
- `frontend/`：Next.js 14 + TypeScript + shadcn/ui 前端（端口 3000），通过 API 调用后端
...
后端 API 访问地址：`http://localhost:8000`；前端访问地址：`http://localhost:3000`。
