    ...
    async def _fetch_agent_card(self, resolver: A2ACardResolver) -> AgentCard:
        ...
        try:
            ...
            if public_card.supportsAuthenticatedExtendedCard:
                try:
                    logger.info(
                        f"尝试获取扩展Agent Card: {self.base_url}{extended_card_path}"
                    )
                    auth_headers = {"Authorization": "Bearer your-auth-token-here"}
    ...
    async def ask(
        ...
        try:
            ...
            response = await self.a2a_client.send_message(
                send_request, http_kwargs={"timeout": 60.0}
            )
            logger.info("收到QA响应")
    ...
    async def batch_ask(
        ...
        for i, query in enumerate(queries):
            try:
                ...
                results.append({"success": False, "question": query, "error": str(e)})

        return results


# 使用示例和测试函数
@pytest.mark.skip(reason="集成测试，需要实际的 A2A 服务器运行")
