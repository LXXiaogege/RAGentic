...
RAGentic 是一个企业级 RAG（检索增强生成）智能问答系统，基于 LangGraph 构建复杂工作流。支持知识库检索、工具调用、多轮对话记忆，并提供同步/流式输出，可选接入
Langfuse 进行全链路观测。
...
config = AppConfig()
pipeline = LangGraphQAPipeline(config)
result = pipeline.ask_sync("什么是 RAG？")  # 同步包装器
print(result["answer"])
```
...
async def main():
    ...
    async for event in pipeline.ask_stream("什么是 RAG？"):
...
pipeline = LangGraphQAPipeline(config)
result = await pipeline.ask("洛杉矶今天天气怎么样？")
```
...
1. `parse_query` - 解析查询参数，设置默认配置
2. `agent_node` - Agent 推理（use_tool=True 时），循环调用工具直到无 tool_calls
3. `tools_node` - 执行 MCP 工具调用（kb_search、天气等）并回填结果
4. `build_context` - 构建最终上下文，整合 KB/LTM/工具结果
5. `generate_answer` - 生成答案（use_tool=False 时的纯 LLM 路径）
6. `update_memory` - 更新分层记忆（STM + LTM）
7. `handle_error` - 异常处理分支
...
```bash
# 方式1: 使用 chainlit CLI（推荐）
PYTHONPATH=. uv run chainlit run chainlit_app.py

# 方式2: 直接运行（需要 Python 3.10+）
PYTHONPATH=. uv run python chainlit_app.py
```
...
#### `ask(query: str, **kwargs) -> Dict[str, Any]` (async)

异步问答接口，执行完整的 RAG 工作流。
...
```python
pipeline = LangGraphQAPipeline(config)
result = pipeline.ask_sync("什么是 RAG？")  # 内部使用 asyncio.run()
```

---

#### `ask_stream(query: str, **kwargs) -> AsyncGenerator[Dict, None]` (async)

异步流式问答接口，逐节点返回执行状态和答案片段。

**参数：** 同 `ask()`

**返回值：** 异步生成器，每次 yield 一个事件字典：

```python
# 节点处理中（每个节点都会 yield 一次）
{"node": "build_context", "status": "processing",
 "state": {"kb_context": "...", "tool_context": "...", "final_context": "..."}}
...
async def main():
    pipeline = LangGraphQAPipeline(config)
    async for event in pipeline.ask_stream("解释深度学习原理"):
        if event.get("status") == "complete":
            print(event["answer"])
...
#### `batch_ask(questions: List[str], **kwargs) -> List[Dict[str, Any]]` (async)
...
```python
results = await pipeline.batch_ask(["问题1", "问题2", "问题3"])
```

**同步包装器：**

```python
results = pipeline.batch_ask_sync(["问题1", "问题2", "问题3"])
```
...
```bash
# 方式1: 使用 chainlit CLI 指定端口
PYTHONPATH=. uv run chainlit run chainlit_app.py --port 7861
...
# 方式3: 使用环境变量
CHAINLIT_PORT=7861 PYTHONPATH=. uv run chainlit run chainlit_app.py
```
